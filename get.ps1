...
# GitHub config
$repo = "RedLazyPanda/Lazy"
$apiUrl = "https://api.github.com/repos/$repo/releases/latest"
$headers = @{
    "User-Agent" = "Lazy-Fetcher"
...
# Add build step for creating the setup
Write-Host "[>] Building the setup..." -ForegroundColor Cyan
try {
    npm run build
...
$asset = $release.assets | Where-Object { $_.name -match "^lazy-.*-setup\.exe$" }
...
try {
    ...
    Write-Host "[>] Thanks For using Lazy" -ForegroundColor Magenta
} catch {
