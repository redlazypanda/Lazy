...
async function Defender() {
  ...
  if (app.isPackaged) {
    ...
    if (result.success) {
      console.log(logo, "Added Lazy to Windows Defender Exclusions")
    } else {
      console.error(logo, "Failed to add Lazy to Windows Defender Exclusions", result.error)
    }
  } else {
...
icon: path.join(__dirname, "../../resources/lazylogo.ico"),
    webPreferences: {
...
app.whenReady().then(() => {
  ...
  electronApp.setAppUserModelId("com.RedLazyPanda.Lazy")
