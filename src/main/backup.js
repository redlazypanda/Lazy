...
ipcMain.handle('create-Lazy-restore-point', async () => {
  const label = `LazyBackup-${getTimestamp()}`
  try {
...
ipcMain.handle('delete-old-Lazy-backups', async () => {
  return new Promise((resolve, reject) => {
    const LazyRoot = `C:\\Lazy`
    if (!fs.existsSync(LazyRoot)) {
      return resolve({ success: true, message: 'Lazy folder does not exist' })
    }

    fs.rm(LazyRoot, { recursive: true, force: true }, (err) => {
      if (err) return reject(err)
      resolve({ success: true, message: 'Lazy folder deleted' })
    })
  })
})
