...
function Clean() {
  ...
  return (
    <RootDiv>
      <div>
        <div className="mb-8">
          <div className="bg-Lazy-card border border-Lazy-border rounded-2xl p-6 backdrop-blur-sm">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-green-500/10 rounded-xl">
                <Icon iconNode={broom} className="text-green-500" size={28} />
              </div>
              <div className="flex-1">
                <h2 className="text-2xl font-bold text-Lazy-text mb-1">System Cleanup</h2>
                <div className="flex items-center gap-2">
                  <p className="text-sm text-Lazy-text-secondary">Last cleaned:</p>
                  <p className="text-sm font-medium text-Lazy-text-secondary">{lastClean}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {cleanups.map(({ id, label, description, script }) => (
            <div
              key={id}
              className="group relative bg-Lazy-card hover:bg-Lazy-border/50 border border-Lazy-border hover:border-Lazy-primary rounded-xl p-6 transition-all duration-200"
            >
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-Lazy-text group-hover:text-Lazy-primary transition-colors duration-200 mb-2">
                  {label}
                </h3>
                <p className="text-Lazy-text-secondary text-sm leading-relaxed">{description}</p>
              </div>
              <Button
                onClick={() => runCleanup(id, script)}
                disabled={loadingId !== null}
                className="w-full flex items-center justify-center gap-2 text-Lazy-text rounded-lg transition-all duration-200 bg-slate-700/50 hover:bg-Lazy-primary border-none"
              >
                {loadingId === id ? (
                  <>
              ...
              {loadingId === id && (
                <div className="absolute inset-0  backdrop-blur-[1px] rounded-xl flex items-center justify-center">
                  <div className="bg-Lazy-card border border-Lazy-border rounded-lg px-4 py-2 flex items-center gap-2">
                    <RefreshCw className="animate-spin text-Lazy-primary" size={18} />
                    <span className="text-sm font-medium text-Lazy-text">
                      Running cleanup...
                    </span>
                  </div>
                </div>
