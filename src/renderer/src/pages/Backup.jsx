...
export default function RestorePointManager() {
  ...
  const handleCreateRestorePoint = async () => {
    setProcessing(true)
    try {
      await invoke({ channel: 'create-Lazy-restore-point' })
    ...
    <RootDiv>
      <div className="h-full max-w-full">
        <div className="flex items-center justify-between mb-4">
          <div className="relative w-64 mt-1 ml-1">
            <Search
              className="absolute left-3 top-1/2 transform -translate-y-1/2 text-Lazy-text-secondary"
              size={16}
            />
            <input
              type="text"
              ...
              className="w-full pl-10 pr-4 py-2 bg-Lazy-card border border-Lazy-border rounded-lg text-Lazy-text placeholder-Lazy-text-secondary focus:outline-none focus:ring-2 focus:ring-Lazy-primary focus:border-transparent transition-colors"
            />
          </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center h-96">
            <Loader2 size={32} className="text-Lazy-primary animate-spin" />
          </div>
        ) : filteredRestorePoints.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center bg-Lazy-card border border-Lazy-border rounded-lg">
            <div className="p-4 bg-Lazy-secondary rounded-full mb-4">
              <Shield size={28} className="text-Lazy-text" />
            </div>
            <h3 className="text-lg font-medium mb-2 text-Lazy-text">No Restore Points Found</h3>
            <p className="text-Lazy-text-secondary max-w-sm mb-4">
              {searchQuery
          ...
          <div className="bg-Lazy-card border border-Lazy-border rounded-lg overflow-hidden">
            <div className="max-h-96 overflow-y-auto">
              <table className="w-full text-sm text-left">
                <thead className="text-xs text-Lazy-text-secondary uppercase bg-Lazy-card sticky top-0">
                  <tr>
                ...
                <tbody>
                  {filteredRestorePoints.map((rp, index) => (
                    <tr key={index} className={`border-t border-Lazy-border`}>
                      <td className="px-6 py-4 font-medium text-Lazy-text">{rp.Description}</td>
                      <td className="px-6 py-4 text-center">
                        <div className="flex justify-center">
                          ...
                          >
                            <RotateCcw size={16} className="text-Lazy-primary" />
                          </Button>
                        </div>
                      </td>
                    </tr>
        ...
        <p className="text-center text-Lazy-text-muted mt-4">
      ...
      >
        {modalState.type === 'restore' && (
          <div className="w-[400px] bg-Lazy-bg border border-Lazy-border rounded-xl shadow-xl">
            <div className="flex items-center justify-between p-4 border-b border-Lazy-border">
              <h3 className="text-lg font-medium text-Lazy-text">Restore System</h3>
              <button
                onClick={() =>
                  !processing && setModalState({ isOpen: false, type: null, restorePoint: null })
                }
                className={`text-Lazy-text-secondary transition-colors ${processing ? 'opacity-50 cursor-not-allowed' : 'hover:text-Lazy-text'}`}
                disabled={processing}
              >
                <X size={20} />
              </button>
            ...
            <div className="p-4">
              <p className="text-Lazy-text mb-4">
      ...
      >
        <div className="w-[400px] bg-Lazy-bg border border-Lazy-border rounded-xl shadow-xl">
          <div className="flex items-center justify-between p-4 border-b border-Lazy-border">
            <h3 className="text-lg font-medium text-Lazy-text">Create Custom Restore Point</h3>
            <button
              onClick={() => {
                if (!processing) {
                  setCustomModalOpen(false)
          ...
          <div className="p-4">
            <input
              ...
              className="w-full px-3 py-2 bg-Lazy-card border border-Lazy-border rounded-lg text-Lazy-text placeholder-Lazy-text-secondary focus:outline-none focus:ring-2 focus:ring-Lazy-primary focus:border-transparent transition-colors mb-4"
              disabled={processing}
            />
            <div className="flex justify-end gap-3">
              <Button
            ...
            <p className="text-sm text-center mt-3 text-Lazy-text-muted">
