  ...
  return (
    <RootDiv>
      <div className="bg-Lazy-bg text-Lazy-text pb-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {utilities.map((util) => (
            <button
              ...
              disabled={running}
              className="bg-Lazy-card border border-Lazy-border p-4 rounded-2xl flex items-center gap-3 hover:border-Lazy-primary transition"
            >
              <div className={`${util.color}`}>{util.icon}</div>
              <div className="text-left">
                <h2 className="font-semibold">Run {util.name}</h2>
                <p className="text-sm text-Lazy-text-secondary">Type: {util.type}</p>
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="mt-8">
          <h2 className="text-xl font-semibold mb-2 text-Lazy-text-secondary">Quick Access</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {quickAccess.map((tool) => (
              <button
                ...
                className="inline-flex items-center gap-2 px-3 py-3.5 text-sm bg-Lazy-card border border-Lazy-border rounded-xl hover:border-Lazy-primary transition text-Lazy-text"
              >
                {tool.icon}
          ...
          <div className="bg-Lazy-card p-6 rounded-2xl border border-Lazy-border text-Lazy-text w-[90vw] max-w-md">
            ...
            {selectedUtility && (
              <>
                ...
                <div className="bg-Lazy-border-secondary border border-Lazy-border p-2 text-sm rounded-md text-Lazy-text-secondary">
