...
export default function DNSPage() {
  ...
  return (
    <RootDiv>
      <div className="bg-Lazy-bg text-Lazy-text pb-10">
        <div className="bg-Lazy-card border border-Lazy-border p-4 rounded-2xl mb-6">
            ...
            <div className="space-y-2">
              {currentDNS.map((dns, index) => (
                <div key={index} className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-Lazy-text-secondary">{dns.servers}</span>
                </div>
              ))}
            </div>
          ) : (
            <div className="flex items-center gap-2 text-sm text-Lazy-text-secondary">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          {dnsProviders.map((provider) => (
            <button
              ...
              disabled={loading}
              className="bg-Lazy-card border border-Lazy-border p-4 rounded-2xl hover:border-Lazy-primary transition text-left"
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <h3 className="font-semibold">
                    ...
                    {provider.recommended && (
                      <span className="text-xs text-Lazy-primary ml-2">Recommended</span>
                    )}
                  </h3>
              ...
              <p className="text-sm text-Lazy-text-secondary">{provider.description}</p>
              {/* <div className="flex flex-wrap gap-1 mt-3">
                {provider.features.map((feature, index) => (
                  <span key={index} className="px-2 py-1 bg-Lazy-border text-xs rounded-md">
                    {feature}
                  </span>
                ))}
              </div> */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-Lazy-border border border-Lazy-border-secondary rounded-lg text-Lazy-text focus:outline-none focus:border-Lazy-primary"
                  />
                </div>
                <div>
        ...
        <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
          <div className="bg-Lazy-card p-6 rounded-2xl border border-Lazy-border text-Lazy-text w-[90vw] max-w-md">
            <h2 className="text-lg font-semibold mb-4">Confirm DNS Change</h2>
            {selectedProvider && (
              <>
                <p className="mb-4">
                  You are about to change your DNS servers to{" "}
                  <span className="text-Lazy-primary font-medium">{selectedProvider.name}</span>.
                </p>
                <div className="bg-Lazy-border-secondary border border-Lazy-border p-3 rounded-md mb-4">
                ...
                <p className="text-sm text-Lazy-text-secondary mb-4">
