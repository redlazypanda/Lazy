    ...
    <RootDiv>
      <div className="min-h-screen w-full pb-24 overflow-y-auto">
        <div className="space-y-8">
          <SettingSection title="Appearance">
            <SettingCard>
              <div className="space-y-4">
                <h3 className="text-base font-medium text-Lazy-text">Theme</h3>
                  ...
                  {themes.map((t) => (
                    <label
                      ...
                      className={`flex items-center justify-center gap-2 cursor-pointer p-3 rounded-lg border transition-all duration-200 hover:scale-105 active:scale-95 ${
                        theme === t.value ? "border-Lazy-primary" : "border-Lazy-border"
                      }`}
                    >
                      <input
                        type="radio"
                        ...
                        className="sr-only"
                      />
                      <span className="text-Lazy-text font-medium">{t.label}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            <SettingCard>
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <h3 className="text-base font-medium text-Lazy-text mb-1">
                    Discord Rich Presence
                  </h3>
                  <p className="text-sm text-Lazy-text-secondary">
                    Show your Lazy activity on Discord
                  </p>
                </div>
                  ...
                  <span
                    className={`text-xs font-medium px-2 py-1 rounded-full ${
                      discordEnabled
                        ...
                        : "text-Lazy-text-secondary bg-Lazy-border-secondary/20"
                    }`}
                  >
          ...
          <SettingSection title="Privacy">
            <SettingCard>
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <h3 className="text-base font-medium text-Lazy-text mb-1">Analytics</h3>
                  <p className="text-sm text-Lazy-text-secondary">
                    Posthog analytics
          ...
          <SettingSection title="Data Management">
            <SettingCard>
              <div className="flex items-center justify-between mb-4">
                <div className="flex-1">
                  <h3 className="text-base font-medium text-Lazy-text mb-1">Legacy Backups</h3>
                  <p className="text-sm text-Lazy-text-secondary">
                    Remove old backup files stored in{" "}
                    <code className="bg-Lazy-border-secondary/20 px-1 py-0.5 rounded text-xs">
                      C:\Lazy\Backup
                    </code>
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between mt-2">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-Lazy-text-secondary">
                    Remove temporary PowerShell files Lazy may leave behind.
                  </p>
                </div>
                <Button
                  variant="secondary"
                  onClick={async () => {
                    await invoke({ channel: "clear-Lazy-cache" })
                    toast.success("Lazy cache cleared successfully!")
                  }}
                >
                  Clear Cache
                </Button>
          ...
          <SettingSection title="Other">
            <SettingCard>
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <h3 className="text-base font-medium text-Lazy-text mb-1">Show tray icon</h3>
                  <p className="text-sm text-Lazy-text-secondary">
                    Enable or disable the Lazy tray icon in the system tray.
                    <span className="inline-flex items-center gap-1 ml-2 text-yellow-500">
                      <span className="w-1.5 h-1.5 bg-yellow-500 rounded-full"></span>
                      Requires restart
                    </span>
          ...
          <SettingSection title="About">
            <SettingCard>
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-base font-medium text-Lazy-text mb-1">Lazy</h3>
                  <p className="text-sm text-Lazy-text-secondary">Version {jsonData.version}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-Lazy-text-secondary">
                    © {new Date().getFullYear()} RedLazyPanda Network
                  </p>
                </div>
              </div>
            </SettingCard>
        ...
        <Modal open={deleteModalOpen} onClose={() => setDeleteModalOpen(false)}>
          <div className="bg-Lazy-card border border-Lazy-border rounded-2xl p-8 shadow-2xl max-w-md w-full mx-4">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  invoke({ channel: "delete-old-Lazy-backups" })
                }}
              >
                Delete
              </Button>
...
const SettingCard = ({ children, className = "" }) => (
  <div className={`bg-Lazy-card border border-Lazy-border rounded-lg p-4 ${className}`}>
    {children}
  </div>
)
...
const SettingSection = ({ title, children }) => (
  <div className="space-y-4">
    <h2 className="text-lg font-semibold text-Lazy-text">{title}</h2>
    {children}
  </div>
)
