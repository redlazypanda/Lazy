...
function Loading({ onLoadingComplete }) {
  ...
  return (
    <div className="flex justify-center items-center h-screen">
      ...
      >
        <motion.div
          className="text-2xl font-medium mb-8 text-Lazy-text"
        ...
        <div className="w-64 h-1 bg-Lazy-accent rounded-full ">
          <motion.div
            className="h-full bg-Lazy-primary"
        ...
        <div className="mt-12 flex space-x-4">
          {steps.map((_, i) => (
            <motion.div
              key={i}
              className={`w-3 h-3 rounded-full ${i === loadingStep ? 'bg-Lazy-primary' : 'bg-Lazy-accent'}`}
              animate={{
