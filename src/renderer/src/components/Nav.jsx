...
function Nav() {
  ...
  return (
    <nav className="h-screen w-52 text-Lazy-text fixed left-0 top-0 flex flex-col py-6 border-r border-Lazy-border-secondary z-40">
      <div className="flex-1 flex flex-col gap-2 px-3 mt-10 relative" ref={containerRef}>
        <div
          className="absolute left-0 w-1 bg-Lazy-primary rounded transition-all duration-300"
          style={{
        ...
        {Object.entries(tabs).map(([id, { label, path }]) => (
          <Button
            ...
            className={clsx(
              ...
              activeTab === id
                ? 'border-transparent text-Lazy-primary'
                : 'text-Lazy-text-secondary hover:bg-Lazy-border-secondary hover:text-Lazy-text border-transparent'
            )}
          >
            <div>{tabIcons[id]}</div>
      ...
      {needsRestart && (
        <button
          className={clsx(
            ...
            'bg-Lazy-card text-Lazy-text border-Lazy-border-secondary hover:bg-Lazy-border-secondary hover:text-Lazy-text'
          )}
      ...
      <div className="flex items-center justify-center gap-2 mt-4 mb-2">
        <a href="https://github.com/RedLazyPanda/Lazy" target="_blank">
          <GithubIcon className="w-5 fill-Lazy-text-secondary" />
        </a>
        <a href="https://discord.com/invite/En5YJYWj3Z" target="_blank">
          <DiscordIcon className="w-5 fill-Lazy-text-secondary" />
        </a>
      </div>
      <p className="text-Lazy-text-secondary text-center">v{info.version}</p>
    </nav>
  )
}
